...
@php
  ...
  $dificultad = old('dificultad', in_array($producto->dificultad, ['BAJA', 'ALTA'], true) ? $producto->dificultad : 'BAJA');
  $pautaComercial = (bool) old('pauta_comercial', $producto->pauta_comercial);
@endphp
...
<section class="flex-grow-1">
  <div class="d-flex align-items-center justify-content-between flex-wrap gap-2 mb-3">
    <div>
      ...
      <div class="text-muted small">Manager de diseño</div>
    </div>

    <a href="{{ route('manager.productos.index') }}" class="btn btn-outline-secondary rounded-0">
      Volver al listado
    </a>
  </div>
  ...
  <div class="editor-layout">
    <div class="editor-panel">
      <div class="card card-form">
        <div class="card-body">
          <form method="POST" id="formMain" action="{{ route('manager.productos.update', $producto) }}">
            @csrf
            @method('PUT')
            ...
            <div class="row g-3 mb-3">
              <div class="col-md-6">
                ...
                <select name="disenador_id" class="form-select rounded-0" required>
                  <option value="" disabled {{ old('disenador_id', $producto->disenador_id) ? '' : 'selected' }}>
                    Seleccione diseñador
                  </option>
                  @foreach($disenadores as $disenador)
                    <option value="{{ $disenador->id }}" @selected((string) old('disenador_id', $producto->disenador_id) === (string) $disenador->id)>
                      {{ $disenador->name }}
                    </option>
                  @endforeach
                </select>
              ...
              <div class="col-md-3">
                <label class="form-label d-block">Dificultad</label>
                <div class="form-check">
                  <input class="form-check-input" type="radio" name="dificultad" id="difBaja" value="BAJA" {{ $dificultad === 'BAJA' ? 'checked' : '' }}>
                  <label class="form-check-label" for="difBaja">Baja</label>
                </div>
                <div class="form-check">
                  <input class="form-check-input" type="radio" name="dificultad" id="difAlta" value="ALTA" {{ $dificultad === 'ALTA' ? 'checked' : '' }}>
                  <label class="form-check-label" for="difAlta">Alta</label>
                </div>
              </div>

              <div class="col-md-3">
                <label class="form-label d-block">Pauta comercial</label>
                <div class="form-check mt-2">
                  <input class="form-check-input" type="checkbox" name="pauta_comercial" id="chkPauta" value="1" {{ $pautaComercial ? 'checked' : '' }}>
                  <label class="form-check-label" for="chkPauta">Sí</label>
                </div>
              </div>
            </div>
            ...
            <div class="editor-actions">
              ...
              <button type="button" class="btn btn-action-return rounded-0" id="btnDevolverEditor">
                Devolver al editor
              </button>
    ...
    <aside class="editor-side">
      ...
      <div class="card">
        ...
        <div class="card-footer bg-white border-top">
          <form method="POST" action="{{ route('manager.productos.mensajes.store', $producto) }}">
            @csrf
...
<div class="modal fade" id="modalDevolverEditor" tabindex="-1" aria-hidden="true">
  <div class="modal-dialog modal-dialog-centered">
    <div class="modal-content rounded-0">
      <div class="modal-header">
        <h5 class="modal-title">Devolver al editor</h5>
        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Cerrar"></button>
      </div>

      <div class="modal-body">
        <label class="form-label">Motivo</label>
        <textarea class="form-control rounded-0" id="motivoDevolucion" rows="3" placeholder="Explica por qué se devuelve al editor..."></textarea>
      ...
      <div class="modal-footer">
        ...
        <button type="button" class="btn btn-action-return rounded-0" id="btnConfirmarDevolverEditor">Devolver</button>
      </div>
    </div>
  </div>
</div>
  ...
  <div class="modal-dialog modal-dialog-centered">
    <div class="modal-content rounded-0">
      ...
      <div class="modal-body">
        ...
        <input type="url" class="form-control rounded-0" id="canvaUrlInput" placeholder="https://www.canva.com/design/...">
...
(() => {
  ...
  const btnDevolverEditor = document.getElementById('btnDevolverEditor');
  const btnConfirmarDevolverEditor = document.getElementById('btnConfirmarDevolverEditor');
  ...
  const modalDevolverEditorEl = document.getElementById('modalDevolverEditor');
  ...
  btnDevolverEditor?.addEventListener('click', () => {
    if (window.bootstrap && modalDevolverEditorEl) {
      bootstrap.Modal.getOrCreateInstance(modalDevolverEditorEl).show();
    }
  });

  btnConfirmarDevolverEditor?.addEventListener('click', () => {
    const motivo = (motivoDevolucion?.value || '').trim();
    if (!motivo) {
      motivoDevolucion?.classList.add('is-invalid');
      return;
    }
