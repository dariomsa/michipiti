...
@php
  ...
  $puedeAprobar = $puedeEditarMultiplataforma && auth()->user() && auth()->user()->hasAnyRole(['editor', 'director']);
  $puedeDragDrop = $puedeEditarMultiplataforma && auth()->user() && auth()->user()->hasAnyRole(['editor', 'director']);
  $plataformaIconMap = [
      'facebook' => 'facebook',
      'instagram' => 'instagram',
      'tiktok' => 'tiktok',
  ...
  $multiplataformaPlataformasMap = $multiplataformaPlataformas
 ...
 // echo json_encode($multiplataformaPlataformasMap);
@endphp
  ...
  <div class="modal-dialog modal-lg modal-dialog-scrollable">
    <div class="modal-content">
        ...
        <form id="slotForm" class="needs-validation" novalidate autocomplete="off">
          ...
          <div class="row g-3">
            <div class="col-md-6">
              ...
              <select class="form-select" id="formPrioridad" name="multiplataforma_prioridad_id" required>
                <option value="">-- Seleccione prioridad --</option>
                @foreach($multiplataformaPrioridades as $prioridad)
            ...
            <div class="col-md-6">
              ...
              <div class="social-grid" id="plataformasGrid">
                @foreach($multiplataformaPlataformas as $plataforma)
                  @php
                  ...
                  <label class="social-check" title="{{ $plataforma->nombre }}">
                    <input
                      ...
                      name="multiplataforma_plataformas_ids[]"
                      value="{{ $plataforma->id }}"
                    >
            ...
            <div class="col-md-6">
              ...
              <select class="form-select" id="formEquipo" name="multiplataforma_equipo_id" required>
                <option value="">-- Seleccione equipo --</option>
                @foreach($multiplataformaEquipos as $equipo)
            ...
            <div class="col-md-6">
              ...
              <select class="form-select" id="formMultiplataformaTipo" name="multiplataforma_tipo_id" required>
                <option value="">-- Seleccione tipo --</option>
                @foreach($multiplataformaTipos as $tipo)
      ...
      <div class="modal-footer">
        ...
        <button type="button" class="btn btn-dark {{ $puedeEditarMultiplataforma ? '' : 'd-none' }}" id="saveSlotBtn" @disabled(! $puedeEditarMultiplataforma)>Guardar</button>
      </div>
    </div>
  </div>
</div>
...
<script>
  ...
  const multiplataformaPlataformasMap = @json($multiplataformaPlataformasMap);
  ...
  const formMultiplataformaTipo = document.getElementById('formMultiplataformaTipo');
  ...
  function buildWeekSignature(items){
    return JSON.stringify(
      (items || []).map(item => [
        item.uid || `${item.source || 'multiplataforma'}:${item.id || 0}`,
        item.fecha || '',
        (item.hora || '').slice(0, 5),
        item.estado || '',
        item.origen || '',
  ...
  function findSlotItemByUid(key, uid){
    return slotItems(slotData[key]).find(item => String(item.uid || `${item.source || 'multiplataforma'}:${item.id}`) === String(uid)) || null;
  }
  ...
  function getSelectedPlataformas(){
    return [...plataformasGrid.querySelectorAll('input[name="multiplataforma_plataformas_ids[]"]:checked')]
      .map(input => Number(input.value))
      .filter(Number.isInteger);
  }
  ...
  function setSelectedPlataformas(values = []){
    ...
    plataformasGrid.querySelectorAll('input[name="multiplataforma_plataformas_ids[]"]').forEach(input => {
      input.checked = selected.has(Number(input.value));
    });
  }
  ...
  function renderCellContent(td, data){
    ...
    td.title = items.map(data => [
      ...
      'Origen: Multiplataforma',
      `Prioridad: ${data.multiplataforma_prioridad_nombre || data.prioridad || 'Sin prioridad'}`,
      `Plataforma: ${data.multiplataforma_plataforma_nombre || 'Sin plataforma'}`,
      `Equipo: ${data.multiplataforma_equipo_nombre || data.seccion || 'Sin equipo'}`,
      `Tipo: ${data.multiplataforma_tipo_nombre || data.tipo_producto_nombre || 'Sin tipo'}`,
  ...
  function renderSlotItemContent(data, compact = false){
    ...
    const tipo = String(data.multiplataforma_tipo_nombre || '').toLowerCase();
    ...
    badge.textContent = data.multiplataforma_tipo_nombre || 'Editorial';

    const meta = document.createElement('div');
    meta.className = 'slot-meta';
    meta.textContent = [
      data.etapa || 'Borrador',
      data.multiplataforma_equipo_nombre || data.seccion || 'Sin equipo',
    ...
    const socialIds = Array.isArray(data.multiplataforma_plataformas_ids)
        ? data.multiplataforma_plataformas_ids
        : (data.multiplataforma_plataforma_id ? [data.multiplataforma_plataforma_id] : []);
    const socialMap = multiplataformaPlataformasMap;
  ...
  function applySearchFilter(){
    ...
    document.querySelectorAll('.slot').forEach(td => {
      ...
      const haystack = items.map(item => [
          ...
          item?.edicion_nombre,
          item?.etapa,
          item?.multiplataforma_prioridad_nombre,
          item?.multiplataforma_plataforma_nombre,
          item?.multiplataforma_equipo_nombre,
          item?.multiplataforma_tipo_nombre,
  ...
  async function loadPeriodistas(){
    const users = await fetchJSON('/multiplataforma/planificador/periodistas');
  ...
  async function loadVideografos(){
    const users = await fetchJSON('/multiplataforma/planificador/videografos');
  ...
  async function loadWeek(){
    ...
    const items = await fetchJSON(`/multiplataforma/planificador/week?week_start=${encodeURIComponent(start)}`);
  ...
  function setFormEditable(mode = 'full'){
    ...
    formMultiplataformaTipo.disabled = readOnly || !fullEditable;
  ...
  function syncEtapaVisibility(){
    ...
    const comercial = isComercialTipo(formMultiplataformaTipo);
  ...
  function syncAuspicioVisibility(){
    const show = isComercialTipo(formMultiplataformaTipo);
  ...
  function toggleActionButtons(existing){
    ...
    saveBtn.classList.toggle('d-none', !puedeEditarMultiplataforma || isLockedStatus);
    ...
    deleteSlotBtn.classList.toggle('d-none', !puedeEditarMultiplataforma || !existing.id || !existing.can_delete);
  ...
  function prepareNewProductSameSlot(){
    ...
    formMultiplataformaTipo.value = formMultiplataformaTipo.options[1]?.value || '';
    formTitle.value = '';
    formDesc.value = '';
    formAuspicio.value = '';
    ...
    saveBtn.classList.toggle('d-none', !puedeEditarMultiplataforma);
    refreshModalLabels();
    syncStatusBySchedule();
  }

  function fillModalForm(existing, fecha, hourValue, allowed, key){
    if(!puedeEditarMultiplataforma && !existing.id){
      return;
    }
    ...
    formPrioridad.value = existing.multiplataforma_prioridad_id || formPrioridad.options[1]?.value || '';
    setSelectedPlataformas(existing.multiplataforma_plataformas_ids || (existing.multiplataforma_plataforma_id ? [existing.multiplataforma_plataforma_id] : []));
    formEquipo.value = existing.multiplataforma_equipo_id || formEquipo.options[1]?.value || '';
    formMultiplataformaTipo.value = existing.multiplataforma_tipo_id || formMultiplataformaTipo.options[1]?.value || '';
    ...
    setFormEditable(!puedeEditarMultiplataforma ? 'readonly' : (existing.origen === 'pauta' ? 'pauta' : 'full'));
  ...
  async function chooseSlotItem(items, fecha, hourValue){
    const options = items.map(item => {
      const tipo = item.multiplataforma_tipo_nombre || 'Tipo';
      const title = item.titulo || item.seccion || `Producto #${item.id}`;
      const uid = item.uid || `${item.source || 'multiplataforma'}:${item.id}`;
    ...
    const result = await Swal.fire({
      ...
      showCancelButton: true,
      showDenyButton: puedeEditarMultiplataforma,
      confirmButtonText: puedeEditarMultiplataforma ? 'Editar' : 'Ver',
  ...
  async function openModal(dayIndex, hourValue, allowed = true){
    ...
    if(!puedeEditarMultiplataforma && items.length === 0){
      return;
    }
  ...
  async function moveSlot(source, target){
    ...
    const res = await fetchJSON('/multiplataforma/planificador/move', {
      method: 'POST',
      body: JSON.stringify({
  ...
  async function saveCurrentForm(){
    ...
    const payload = {
      ...
      hora: formTime.value,
      multiplataforma_prioridad_id: formPrioridad.value ? Number(formPrioridad.value) : null,
      multiplataforma_plataformas_ids: getSelectedPlataformas(),
      multiplataforma_equipo_id: formEquipo.value ? Number(formEquipo.value) : null,
      multiplataforma_tipo_id: formMultiplataformaTipo.value ? Number(formMultiplataformaTipo.value) : null,
      titulo: formTitle.value.trim(),
      descripcion: formDesc.value.trim(),
      auspicio: isComercialTipo(formMultiplataformaTipo) ? formAuspicio.value.trim() : null,
    ...
    const res = await fetchJSON('/multiplataforma/planificador/store', {
      method: 'POST',
  ...
  if(approveBtn){
    approveBtn.addEventListener('click', async () => {
      ...
      try{
        const res = await fetchJSON('/multiplataforma/planificador/aprobar', {
          method: 'POST',
  ...
  deleteSlotBtn?.addEventListener('click', async () => {
    ...
    try{
      const res = await fetchJSON(`/multiplataforma/planificador/${propuestaId}`, {
        method: 'DELETE'
      });
  ...
  formMultiplataformaTipo?.addEventListener('change', syncAuspicioVisibility);
