...
@section('title', 'Listado Multiplataforma')

@push('styles')
<style>
  .multiplataforma-page{
    --ink:#0e1726;
    --blue:#0b3d6b;
    --blue2:#16578f;
    --paper:#f4f1ea;
  ...
  .multiplataforma-inner{max-width:1180px;margin:0 auto;}
  .multiplataforma-top{border-bottom:3px solid var(--blue);padding-bottom:14px;margin-bottom:18px;}
  .multiplataforma-brand{color:var(--blue);font-size:13px;font-weight:800;letter-spacing:.22em;text-transform:uppercase;}
  .multiplataforma-sub{color:var(--muted);font-size:14px;line-height:1.25;max-width:760px;}

  .multiplataforma-stats{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:10px;margin:20px 0 16px;}
  .multiplataforma-stat{background:var(--card);border:1px solid var(--line);border-radius:10px;padding:12px 14px;}
  .multiplataforma-stat b{display:block;font-family:Georgia, 'Times New Roman', serif;font-size:26px;line-height:1;color:var(--blue);}
  .multiplataforma-stat.com b{color:var(--com);}
  .multiplataforma-stat.radio b{color:var(--green);}
  .multiplataforma-stat span{color:var(--muted);display:block;font-size:11px;font-weight:700;letter-spacing:.06em;text-transform:uppercase;}

  .multiplataforma-search{background:#fff;border:1px solid var(--line);border-radius:10px;height:34px;padding:6px 12px;width:100%;font-size:14px;}
  ...
  #multiplataformaEditModal .modal-dialog{max-width:720px;width:calc(100% - 1rem);}
  #multiplataformaEditModal .modal-content,
  #multiplataformaEditModal form{max-width:100%;min-width:0;overflow-x:hidden;}
  #multiplataformaEditModal .modal-header,
  #multiplataformaEditModal .modal-footer{padding:.55rem .75rem;}
  #multiplataformaEditModal .modal-body{overflow-x:hidden;padding:.65rem .75rem;}
  #multiplataformaEditModal .row.g-3{--bs-gutter-x:.55rem;--bs-gutter-y:.45rem;}
  #multiplataformaEditModal .row > [class*="col-"]{min-width:0;}
  #multiplataformaEditModal .form-label{font-size:.78rem;font-weight:600;margin-bottom:.25rem;}
  #multiplataformaEditModal .form-control,
  #multiplataformaEditModal .form-select{border-radius:.35rem;font-size:.84rem;height:31px;min-height:31px;padding:.22rem .5rem;}
  #multiplataformaEditModal textarea.form-control{height:auto;line-height:1.2;min-height:56px;padding-bottom:.35rem;padding-top:.35rem;}
  #multiplataformaEditModal .btn{font-size:.82rem;padding:.32rem .62rem;}
  ...
  @media (max-width: 767.98px){
    .multiplataforma-page{margin:0;padding:18px 12px 34px;}
...
@php
  ...
  $chipUrl = function (array $override = []) use ($filters) {
      return route('multiplataforma.index', array_filter(array_merge($filters, $override), fn ($value) => $value !== '' && $value !== 0 && $value !== null));
  };
      ...
      ->mapWithKeys(function ($producto) {
          return [
              $producto->id => [
                  ...
                  'multiplataforma_plataformas_ids' => collect($producto->multiplataforma_plataformas_ids ?? ($producto->multiplataforma_plataforma_id ? [$producto->multiplataforma_plataforma_id] : []))->map(fn ($id) => (int) $id)->values()->all(),
...
<div class="multiplataforma-page">
  <div class="multiplataforma-inner">
    <header class="multiplataforma-top">
      <div class="multiplataforma-brand">EL COMERCIO · MULTIPLATAFORMA</div>
      <div class="multiplataforma-sub">Vista de productos planificados para Multiplataforma: prioridad, plataforma, equipo responsable y tipo de contenido.</div>
    </header>

    <div class="multiplataforma-stats">
      <div class="multiplataforma-stat"><b>{{ $stats['total'] }}</b><span>Productos</span></div>
      <div class="multiplataforma-stat"><b>{{ $stats['editorial'] }}</b><span>Editorial</span></div>
      <div class="multiplataforma-stat com"><b>{{ $stats['comercial'] }}</b><span>Comercial</span></div>
      <div class="multiplataforma-stat radio"><b>{{ $stats['radio'] }}</b><span>Radio</span></div>
    </div>

    <form method="GET" action="{{ route('multiplataforma.index') }}">
      <input class="multiplataforma-search" type="search" name="q" value="{{ $filters['q'] }}" placeholder="Buscar producto, equipo, responsable...">
    </form>
    ...
    @forelse($productos->getCollection()->groupBy(fn ($producto) => optional($producto->fecha)->format('Y-m-d') ?: 'sin-fecha') as $fechaKey => $items)
      ...
      <section class="date-group">
        ...
        @foreach($items as $producto)
          @php
            $tipoNombre = $producto->multiplataformaTipo?->nombre ?? 'Editorial';
            $tipoClass = strcasecmp($tipoNombre, 'Comercial') === 0 ? 'com' : (strcasecmp($tipoNombre, 'Radio') === 0 ? 'radio' : '');
            $plataformaIds = collect($producto->multiplataforma_plataformas_ids ?? ($producto->multiplataforma_plataforma_id ? [$producto->multiplataforma_plataforma_id] : []))->map(fn ($id) => (int) $id);
            ...
            $puedeMarcarMetricool = $puedeEditarMultiplataforma && ! $esMichipiti && ! $metricoolProgramado;
          ...
          <div class="mrow">
            <div class="mtime">
              ...
              <span>{{ $plataformaNombres->first() ?? 'Multiplataforma' }}</span>
            </div>

            <article
              class="mcard {{ $tipoClass }} {{ $puedeEditarMultiplataforma && ! $esMichipiti ? 'is-clickable' : 'is-locked' }}"
              @if($puedeEditarMultiplataforma && ! $esMichipiti) data-edit-product="{{ $producto->id }}" @endif
            >
              ...
              <div class="tags">
                @if($producto->multiplataformaPrioridad)
                  <span class="tag format">{{ $producto->multiplataformaPrioridad->nombre }}</span>
                @endif
              ...
              <div class="meta">
                <span><b>Equipo:</b> {{ $producto->multiplataformaEquipo?->nombre ?? $producto->seccion ?? 'Sin equipo' }}</span>
    ...
    @empty
      <div class="empty">No hay productos multiplataforma con estos filtros.</div>
    @endforelse
...
<div class="modal fade" id="multiplataformaEditModal" tabindex="-1" aria-labelledby="multiplataformaEditModalLabel" aria-hidden="true">
  <div class="modal-dialog modal-lg modal-dialog-scrollable">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="multiplataformaEditModalLabel">Editar producto multiplataforma</h5>
        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Cerrar"></button>
      </div>
      <div class="modal-body">
        <form id="multiplataformaEditForm" class="needs-validation" novalidate>
          ...
          <div class="row g-3">
            ...
            <div class="col-md-6">
              ...
              <select class="form-select" id="editMultiplataformaTipo" required>
      ...
      <div class="modal-footer">
        ...
        @if($puedeEditarMultiplataforma)
          <button type="button" class="btn btn-dark" id="saveMultiplataformaEditBtn">Guardar</button>
        @endif
      </div>
    </div>
  </div>
...
@push('scripts')
<script>
  const multiplataformaEditItems = @json($editItems);
  const canEditDateTime = @json($canEditDateTime);
  const puedeEditarMultiplataforma = @json($puedeEditarMultiplataforma);
  const multiplataformaEditModalEl = document.getElementById('multiplataformaEditModal');
  const multiplataformaEditModal = new bootstrap.Modal(multiplataformaEditModalEl);
  const multiplataformaEditForm = document.getElementById('multiplataformaEditForm');
  const saveMultiplataformaEditBtn = document.getElementById('saveMultiplataformaEditBtn');
  ...
  const editMultiplataformaTipo = document.getElementById('editMultiplataformaTipo');
  ...
  async function ensureEditUsersLoaded(){
    ...
    const [periodistas, videografos] = await Promise.all([
      fetchJSON('/multiplataforma/planificador/periodistas'),
      fetchJSON('/multiplataforma/planificador/videografos'),
    ]);
  ...
  function syncEditAuspicioVisibility(){
    const show = isComercialTipo(editMultiplataformaTipo);
  ...
  function syncEditEtapaVisibility(){
    ...
    const comercial = isComercialTipo(editMultiplataformaTipo);
  ...
  async function openEditModal(productId){
    if(!puedeEditarMultiplataforma) return;

    const item = multiplataformaEditItems[String(productId)] || multiplataformaEditItems[productId];
    if(!item) return;
    ...
    document.getElementById('editPrioridad').value = item.multiplataforma_prioridad_id || '';
    setEditPlataformas(item.multiplataforma_plataformas_ids || []);
    document.getElementById('editEquipo').value = item.multiplataforma_equipo_id || '';
    editMultiplataformaTipo.value = item.multiplataforma_tipo_id || '';
    ...
    multiplataformaEditForm.classList.remove('was-validated');
    syncEditPlataformasValidation(false);
    multiplataformaEditModal.show();
  }
  ...
  document.querySelectorAll('[data-metricool-id]').forEach(input => {
    ...
    input.addEventListener('change', async () => {
      ...
      try{
        const res = await fetchJSON(`/multiplataforma/listado/${input.dataset.metricoolId}/metricool`, {
          method: 'POST',
          body: JSON.stringify({})
        });
  ...
  editMultiplataformaTipo?.addEventListener('change', syncEditAuspicioVisibility);
  editMultiplataformaTipo?.addEventListener('change', syncEditEtapaVisibility);

  saveMultiplataformaEditBtn?.addEventListener('click', async () => {
    if(!puedeEditarMultiplataforma) return;

    const hasPlataformas = syncEditPlataformasValidation(true);
    if(!hasPlataformas || !multiplataformaEditForm.checkValidity()){
      multiplataformaEditForm.classList.add('was-validated');
      return;
    }

    saveMultiplataformaEditBtn.disabled = true;
    saveMultiplataformaEditBtn.textContent = 'Guardando...';

    try{
      await fetchJSON('/multiplataforma/planificador/store', {
        method: 'POST',
        body: JSON.stringify({
          id: Number(document.getElementById('editId').value),
          fecha: document.getElementById('editFecha').value,
          hora: document.getElementById('editHora').value,
          multiplataforma_prioridad_id: Number(document.getElementById('editPrioridad').value),
          multiplataforma_plataformas_ids: getEditPlataformas(),
          multiplataforma_equipo_id: Number(document.getElementById('editEquipo').value),
          multiplataforma_tipo_id: Number(editMultiplataformaTipo.value),
          titulo: document.getElementById('editTitulo').value.trim(),
          descripcion: document.getElementById('editDescripcion').value.trim(),
          auspicio: isComercialTipo(editMultiplataformaTipo) ? editAuspicio.value.trim() : null,
      ...
      saveMultiplataformaEditBtn.disabled = false;
      saveMultiplataformaEditBtn.textContent = 'Guardar';
    }
  });
</script>
@endpush
