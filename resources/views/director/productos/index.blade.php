          ...
          <tbody>
            @forelse($productos as $producto)
              @php
                ...
                $esEditor = auth()->user()?->hasAnyRole(['editor', 'director']);
                ...
                $puedeAprobar = $esEditor && $producto->estado === 'FINALIZADO';
              @endphp
              ...
              <tr>
                ...
                <td style="padding:16px;">
                  @if($puedeAprobar)
                    <button type="button"
                       class="btn btn-sm action-btn-approve me-1 js-approve-btn"
                       style="border-radius:0;"
                       title="Aprobar"
                       aria-label="Aprobar"
                       data-action="{{ route($routeBase.'.approve', $producto->id) }}"
                       data-canva-url="{{ $producto->canva_url }}"
                       data-producto-titulo="{{ $producto->titulo }}">
    ...
    <div class="modal-content rounded-0">
      <form method="POST" id="approveForm">
        ...
        <div class="modal-body">
          <div class="text-muted mb-2" style="font-size:.9rem;" id="approveModalText">
        ...
        <div class="modal-footer">
          ...
          <button type="submit" class="btn btn-success rounded-0">Aceptar</button>
        </div>
      </form>
    </div>
  </div>
...
(function () {
  ...
  document.addEventListener('click', function (event) {
    const button = event.target.closest('.js-approve-btn');
    if (!button || !approveForm) return;
