...
@section('content')
@php
  $searchColClass = $canFilterResponsable ? 'col-12 col-xl-3 col-lg-4' : 'col-12 col-xl-4 col-lg-4';
  $sectionColClass = 'col-12 col-xl-2 col-lg-2';
  $videographerColClass = 'col-12 col-xl-2 col-lg-2';
  $stateColClass = 'col-12 col-xl-1 col-lg-2';
  $dateColClass = $canFilterResponsable ? 'col-12 col-xl-1 col-lg-2' : 'col-12 col-xl-2 col-lg-2';
  $submitColClass = 'col-12 col-xl-1 col-lg-12 d-grid';
@endphp
<section class="flex-grow-1">
  <div class="d-flex align-items-center justify-content-between flex-wrap gap-2 mb-3">
    <h1 class="page-title mb-0">Listado de Audiovisuales</h1>
  </div>
        ...
        <div class="row g-2 align-items-end filters-row">
          ...
          @if($canFilterResponsable)
            <div class="col-12 col-xl-2 col-lg-2">
              ...
              <select class="form-select" name="responsable" style="border-radius:0;">
                <option value="">Todos</option>
                @foreach($responsables as $responsable)
                  <option value="{{ $responsable->id }}" {{ (string)($responsableFiltro ?? '') == (string)$responsable->id ? 'selected' : '' }}>
                    {{ $responsable->name }}
                  </option>
                @endforeach
              </select>
          ...
          <div class="{{ $videographerColClass }}">
            <label class="form-label mb-1">Videógrafo</label>
            <select class="form-select" name="videografo" style="border-radius:0;">
              <option value="">Todos</option>
              @foreach($videografos as $videografo)
                <option value="{{ $videografo->id }}" {{ (string)($videografoFiltro ?? '') == (string)$videografo->id ? 'selected' : '' }}>
                  {{ $videografo->name }}
                </option>
              @endforeach
            </select>
        ...
        <table class="table mb-0 align-middle">
          ...
          <tbody>
            @forelse($audiovisuales as $audiovisual)
              @php
                $dotColor = '#9e9e9e';
                $label = $audiovisual->estado;
                $esEditor = auth()->user()?->hasRole('editor');

                if ($audiovisual->estado === 'APROBADO') $dotColor = '#2e7d32';
                if ($audiovisual->estado === 'EN_REVISION') $dotColor = '#f9a825';
                if ($audiovisual->estado === 'EN_DISENO') $dotColor = '#2563eb';
                if ($audiovisual->estado === 'BORRADOR') $dotColor = '#9e9e9e';
                if ($audiovisual->estado === 'DEVUELTO') $dotColor = '#ef6c00';
                if ($audiovisual->estado === 'RECHAZADO') $dotColor = '#c62828';
              ...
              <tr>
                <td style="padding:16px;">
                  ...
                  @if($audiovisual->hashtags)
                    <div class="text-muted mt-1" style="font-size:.8rem;">
                      {{ \Illuminate\Support\Str::limit($audiovisual->hashtags, 80) }}
                    </div>
                  @endif
                </td>
                ...
                <td style="padding:16px;">
                  {{ optional($audiovisual->user)->name ?: '-' }}
                </td>

                <td style="padding:16px;">
                  {{ optional($audiovisual->disenador)->name ?: '-' }}
                </td>
