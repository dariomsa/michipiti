...
<div class="propuestas-wrap">
  ...
  <div class="mb-3 d-flex flex-wrap gap-3 small text-muted">
    <span><span class="legend-dot" style="background:#dcfce7;border:1px solid #22c55e;"></span>Aprobado / Finalizado</span>
    <span><span class="legend-dot" style="background:#ffedd5;border:1px solid #fb923c;"></span>Pendiente</span>
    <span><span class="legend-dot" style="background:#fef3c7;border:1px solid #f59e0b;"></span>Carrusel</span>
...
<script>
  ...
  toCarruselBtn?.addEventListener('click', async () => {
    ...
    if(!responsableId){
      await showError('Selecciona un responsable antes de convertir a pauta.');
      return;
    }

    const confirmation = await confirmAction('¿Convertir este producto a pauta y asignarlo al responsable?', 'Cambiar');
    if(!confirmation.isConfirmed){
      return;
    }
