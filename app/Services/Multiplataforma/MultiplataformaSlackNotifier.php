...
namespace App\Services\Multiplataforma;
...
class MultiplataformaSlackNotifier
{
    ...
    public function formatHeader(MultiplataformaProducto $producto): string
    {
        $titulo = $producto->titulo ?: ('Producto Multiplataforma #'.$producto->id);
        $lider = $producto->user?->name ?? 'Sin lider';
        $responsable = $producto->responsable2?->name ?? 'Sin responsable';

        return "*{$titulo}* (Multiplataforma ID: {$producto->id})\nLider: {$lider}\nResponsable: {$responsable}";
    }
     ...
     * @return list<int>
     */
    public function involucradosUserIds(MultiplataformaProducto $producto): array
    {
        $ids = [
            $producto->user_id,
        ...
        $movIds = DB::table('multiplataforma_movimientos')
            ->where('multiplataforma_producto_id', $producto->id)
            ->pluck('user_id')
            ->filter()
    ...
    public function notifyInvolucrados(MultiplataformaProducto $producto, string $texto, ?int $actorUserId = null): void
    {
        ...
        Log::info('[SLACK DEBUG] destinatarios movimiento/multiplataforma', [
            'multiplataforma_producto_id' => $producto->id,
    ...
    public function mentionsInvolucrados(MultiplataformaProducto $producto): string
    {
        $ids = $this->involucradosUserIds($producto);
